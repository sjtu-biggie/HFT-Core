...
# HFT System - Start All Services
# Starts all services including placeholders (WebSocket Bridge and Control API)
...
cleanup() {
    ...
    if [ -f "$PID_FILE" ]; then
        ./scripts/stop_services.sh "$PID_FILE"
    fi
    exit 0
}
...
echo "================================================"
echo "HFT System - Starting All Services"
echo "================================================"
...
ALL_PORTS=(5556 5557 5558 5559 8080 8081)
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "    - WebSocket Bridge (placeholder)"
            echo "    - Control API (placeholder)"
            echo ""
...
print_status "Starting all services with mock data for ${MOCK_DURATION}s at ${MOCK_FREQUENCY}Hz"
...
# 2. Start Support Services (placeholders, independent)
...
# 4. Start Strategy Engine (consumes market data, produces signals)
start_service "strategy_engine" "./strategy_engine"

# Small delay to ensure strategy is ready
sleep 2

# 5. Start Order Gateway (consumes signals, produces executions)
start_service "order_gateway" "./order_gateway"

# Small delay to ensure gateway is ready
sleep 2

# 6. Start Position & Risk Service (consumes executions, tracks positions)
...
print_status "==============="
print_status "Core Services (Trading):"
print_status "  ✓ Low Latency Logger    - Centralized logging"
print_status "  ✓ Mock Data Generator   - Market data simulation"
print_status "  ✓ Strategy Engine       - Trading signal generation"
print_status "  ✓ Order Gateway         - Order execution simulation"
print_status "  ✓ Position & Risk       - Portfolio tracking"
echo

print_status "Support Services (Placeholders):"
print_status "  ✓ WebSocket Bridge      - Future: Web dashboard data feed"
print_status "  ✓ Control API           - Future: REST API for system control"
echo
...
while true; do
    ...
    if [ $services_running -lt $total_services ] && [ $services_running -ge 6 ]; then
...
# Clean exit
print_status "All services monitoring completed"